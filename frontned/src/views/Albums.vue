<template>
  <div class="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 pb-4 custom-scrollbar">
    <div class="flex items-center justify-between py-4">
      <h2 class="text-xl font-bold text-gray-900">专辑</h2>
    </div>

    <div v-if="loading && albums.length === 0" class="flex items-center justify-center py-20">
      <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900 dark:border-white"></div>
    </div>

    <div v-else-if="albums.length === 0" class="text-center py-20 text-gray-400">
      暂无专辑
    </div>
      ...
      >
        <div class="relative aspect-square rounded-xl overflow-hidden shadow-md group-hover:shadow-xl transition-all duration-300">
          <img
            ...
            :alt="album.title"
            loading="lazy"
            @error="$event.target.src = `https://picsum.photos/seed/${album.id}/300/300`"
    ...
    <div v-if="loading && albums.length > 0" class="flex items-center justify-center py-4">
...
const fetchAlbums = async () => {
  ...
  try {
    ...
    if (page.value > 1) {
      albums.value = [...albums.value, ...results]
    } else {
