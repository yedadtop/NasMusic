...
class BatchScrapeLyricsView(APIView):
    ...
    def post(self, request):
        ...
        def run_batch_scrape():
            ...
            # 筛选出当前数据库中没有歌词的歌曲
            tracks_to_scrape = Track.objects.filter(lyrics__isnull=True) | Track.objects.filter(lyrics__exact='')

            task.total_files = tracks_to_scrape.count()
            task.processed_files = 0
            task.status = 'running'
            task.save()
            ...
            task.current_file = ''
            task.save()
            print(f"[批量歌词刮削] 完成: 成功 {success_count} 首, 失败 {failed_count} 首")

        threading.Thread(target=run_batch_scrape, daemon=True).start()

        return Response({
            "message": "批量歌词刮削任务已在后台启动",
            "task_id": task_id
        }, status=status.HTTP_202_ACCEPTED)